...
export class DashboardService {
  ...
  async getTenantStatusWidgets(params: { pg_id: number }) {
    ...
    const enrichedTenants = (tenants || []).map((tenant) => {
      ...
      const statusEnriched = this.tenantStatusService.enrichTenantsWithStatus([tenantWithSummary])[0] as Record<
        string,
        unknown
      >;
      ...
      const {
        ...
        proof_documents?: unknown;
        profile_photo?: unknown;
        id_proof?: unknown;
        images?: unknown;
        rent_payments?: unknown;
        advance_payments?: unknown;
        refund_payments?: unknown;
      };
      void proof_documents;
      void profile_photo;
      void id_proof;
      void images;
      void rent_payments;
      void advance_payments;
      void refund_payments;
    ...
    const tenantsWithoutAdvanceEnriched = this.tenantStatusService.getTenantsWithoutAdvance(
      enrichedTenants as unknown[],
    );
  ...
  async getTicketDashboardStats(params: { pg_id: number }) {
    const where: Prisma.tenant_ticketsWhereInput = { pg_id: params.pg_id, is_deleted: false };
    ...
    const [
      ...
      this.prisma.tenant_tickets.findMany({
        ...
        select: {
          ...
          category: true,
          created_at: true,
          tenants: { select: { s_no: true, name: true } },
          _count: { select: { tenant_ticket_comments: { where: { is_deleted: false } } } },
        },
      }),
      // Unread tickets (tickets with no owner comments or only tenant comments)
      this.prisma.tenant_tickets.findMany({
        where,
        orderBy: { created_at: 'desc' },
        take: 10,
    ...
    // A ticket is considered unread if it has no comments from the owner side
    // We'll check if there are any comments with sender_type != 'TENANT'
    ...
    return ResponseUtil.success(stats, 'Dashboard ticket statistics fetched successfully');
  }
}
