...
export class TenantService {
  ...
  async create(createTenantDto: CreateTenantDto) {
    ...
    let bedForAllocation: {
      s_no: number;
      room_id: number | null;
      pg_id: number | null;
      bed_price: Prisma.Decimal | null;
    } | null = null;
    ...
    const tenant = await this.prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      const resolvedRoomId =
        createTenantDto.room_id ??
        (bedForAllocation && bedForAllocation.room_id ? bedForAllocation.room_id : undefined);
      ...
      if (
        createTenantDto.bed_id &&
        bedForAllocation &&
        createdTenant.pg_id &&
        createdTenant.room_id &&
        createdTenant.bed_id
      ) {
        await tx.tenant_allocations.create({
          data: {
  ...
  async findAll(params: {
    ...
    const tenants = await this.prisma.tenants.findMany({
      ...
      include: {
        pg_locations: {
          select: { s_no: true, location_name: true, address: true, rent_cycle_type: true, rent_cycle_start: true },
        },
    ...
    const enriched = tenants.map((tenant) => {
      ...
      return {
        ...
        rent_cycle: rentSummary.current_cycle,
        unpaid_months: rentSummary.unpaid_months,
        periods: rentSummary.periods,
    ...
    let filtered: Enriched[] = enriched;

    if (pending_rent) {
      filtered = this.tenantStatusService.getTenantsWithPendingRent(filtered as unknown[]) as Enriched[];
    }
    if (pending_advance) {
      filtered = this.tenantStatusService.getTenantsWithoutAdvance(filtered as unknown[]) as Enriched[];
    }
    if (partial_rent) {
      filtered = this.tenantStatusService.getTenantsWithPartialRent(filtered as unknown[]) as Enriched[];
    }

    const activeTenants = enriched.filter((t) => (t as Record<string, unknown>).status === 'ACTIVE');
    const pendingTotal = activeTenants.reduce((s, t) => s + Number((t as Record<string, unknown>).pending_due_amount || 0), 0);
    const partialTotal = activeTenants.reduce((s, t) => s + Number((t as Record<string, unknown>).partial_due_amount || 0), 0);
    const pendingCount = activeTenants.filter((t) => Number((t as Record<string, unknown>).pending_due_amount || 0) > 0).length;
    ...
    return ResponseUtil.success(
      {
        ...
        summary: {
          ...
          partial_total: Math.round(partialTotal * 100) / 100,
  ...
  async findOne(id: number) {
    ...
    const allocationsForSnapshot = ((tenant as { tenant_allocations?: TenantAllocationSummary[] }).tenant_allocations ||
      []) as TenantAllocationSummary[];
  ...
  async update(id: number, updateTenantDto: UpdateTenantDto) {
    ...
    if (isCheckInChanging || isRoomChanging || isBedChanging) {
      const [hasRentPayments, hasAdvance, hasRefund, hasBills] = await Promise.all([
        ...
        this.prisma.advance_payments.count({
          where: { tenant_id: id, is_deleted: false, status: { not: 'VOIDED' } },
        }),
    ...
    if (effectiveCheckInDate && effectiveCheckOutDate && effectiveCheckInDate > effectiveCheckOutDate) {
      throw new BadRequestException(
        `Check-in date must be the same as or before check-out date. Check-in date: ${
          effectiveCheckInDate

            .toISOString()

            .split('T')[0]
        }, Check-out date: ${effectiveCheckOutDate.toISOString().split('T')[0]}`,
      );
    }
    ...
    if (updateTenantDto.proof_documents !== undefined) {
      const oldDocuments = (
        Array.isArray(existingTenant.proof_documents) ? existingTenant.proof_documents : []
      ) as string[];

      const newDocuments = (
        Array.isArray(updateTenantDto.proof_documents) ? updateTenantDto.proof_documents : []
      ) as string[];
  ...
  private getUnpaidMonthsWithCycleDates(
    ...
    const cycles = (tenantCycles || [])
      ...
      .sort(
        (a: TenantRentCycleSummary, b: TenantRentCycleSummary) => a.cycle_start.getTime() - b.cycle_start.getTime(),
      );
  ...
  private getCalendarCyclePeriod(startDate: Date): CyclePeriod {
    ...
    const cycleStart =
      startDate.getDate() === 1 ? new Date(year, month, 1) : new Date(year, month, startDate.getDate());
